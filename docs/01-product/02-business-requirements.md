...
> **定案 · 2026-09-08**：第一版由投研发布四类 Profile 固定指标权重，机器可读来源为
> `config/policy/evaluation/v1.yaml`。固定权重仍以 OOS 有效性检验为发布前置，不能让未通过
> 检验的指标进入 Score。

> **“权重已配置”与“指标可进入评分”是两件事。** 固定权重解决配置问题，OOS 检验解决
> 使用资格问题。缺少后者时，即使权重已经定案也不得产出 Score。

| 情形 | 处置 |
|---|---|
| 有效性检验已产出 | 按 Profile 固定权重计算；缺失项按原比例重归一 |
| **有效性检验尚未产出** | **Score 不可投产** —— 不得用固定权重绕过检验 |
...
> **检验样本须划分 IS/OOS** —— 在全历史上做一次检验后就评估或优化权重，等于把权重拟合到了历史上，与直接拍权重的差别只是多了一层数据包装。
...
> **已定案 · 2026-09-08**：第一阶段**不实现权重优化**（`Weight ∝ effectiveness`）。
>
> **依据**：第一版权重已由投研给定；优化权重仍需要
> `Factor Effectiveness Score = f(IC, ICIR, Rank IC, t-stat, stability, redundancy)`
> 的独立论证，不能在 Plan-2 内隐式替换已发布 Policy。
>
> **推进路径**：先让第一版运行并积累数据，再用实证结果（等权 vs 各种加权方案的 OOS 表现对比）来决定第二版的形式。这是 `OPEN-13` 原文「待第一版运行数据积累后确定」的落实。

> **关于权重**：权重属于 `Scoring Version` 配置；有效性检验决定指标是否有资格使用，
> 不自动改写投研发布的权重。调整权重必须发布新 Policy Version。
...
| **30** | **第一版采用 Profile 固定权重，OOS 检验决定使用资格** | 权重由投研发布；检验未产出前 Score 不可投产 | 投研 | 2026-09-08 | Plan-2 设计 §3.5–3.6 |
...
| ~~P1-23~~ | ~~各 Evaluation Profile 内部的具体权重分配~~ —— **已定案**：采用 `PROFILE_FIXED_V1`，详见 Evaluation Policy；OOS 检验决定指标使用资格 | §5.2.1.1 | — | ✅ 已定案 2026-09-08 |
| P1-24 | Fund Tier 是否追加绝对门槛（未达标不得进 A+/A） | §16.3.1 | `06` | 投研 |

> **P1-23 的最终状态**：2026-08-27 的“有效因子内等权”方案已被 2026-09-08 的
> `PROFILE_FIXED_V1` 替代。有效性检验仍是发布前置，但不再自动生成权重。
...
| 版本 | 日期 | 变更内容 | 上游依赖 |
|---|---|---|---|
| **v2.9** | 2026-09-08 | **P1-23 替代决策**。第一版评分改用版本化固定 Profile 权重；OOS 有效性检验决定指标使用资格但不自动生成权重；Benchmark 纳入 Plan-2 最小闭环 | Plan-2 设计 v1.0 |
